...
import { ArrowLeft, Volume2, Loader2, Play, Pause, Download, RefreshCw } from 'lucide-react';
        ...
        <div className="max-w-3xl mx-auto px-4 py-6">
          ...
          <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-6 mb-4">
            ...
            <div className="flex gap-2 mb-3">
              {PRESETS.map((p) => (
                <button key={p.label} onClick={() => setText(p.text)} className="px-3 py-1.5 rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)] text-xs hover:border-primary">
                  {p.label}
                </button>
              ))}
            </div>

            <textarea value={text} onChange={(e) => setText(e.target.value)} rows={8} placeholder="방송할 내용을 입력하세요...&#10;&#10;예: 마을 주민 여러분 안녕하십니까. 내일 오전 10시에 마을회관 앞에서 환경 정화 활동이 있습니다. 많은 참여 부탁드립니다." className="w-full px-4 py-3 bg-[var(--color-bg)] border border-[var(--color-border)] rounded-xl resize-none focus:outline-none focus:border-primary mb-3" />
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-4">
              <label className="block">
                ...
                <select value={voice} onChange={(e) => setVoice(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)] text-sm">
